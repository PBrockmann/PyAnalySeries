...
def create_tree_widget():
    ...
    tree_widget.setColumnWidth(4, 200)
    tree_widget.setColumnWidth(5, 100)
    tree_widget.setColumnWidth(6, 100)
...
def define_interpolation():
    ...
    if not ((len(itemInterpolations_selected) == 1 and len(itemSeries_selected) == 2) or
            (len(itemInterpolations_selected) == 0 and len(itemSeries_selected) == 2)):
        main_window.statusBar().showMessage('Please select 2 series (reference, serie to interpolate) and optionnaly 1 existing INTERPOLATION', 5000)
        return
    ...
    if len(itemInterpolations_selected) == 1:
        items.insert(0, itemInterpolations_selected[0])
    ...
    if Id_interpolationWindow in open_interpolationWindows:
        ...
        interpolationWindow = defineInterpolationWindow(Id_interpolationWindow, open_interpolationWindows, items, add_item_tree_widget)
